<template>
    <Modal :open="open" accept="Save" header="Create Job" @accepted="accept()" @closed="close()">
        <form ref="jobform" @submit.prevent="save()">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="svct" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Remove
                        Backups After</label>
                    <select v-model="job.garbage.quantity" id="svct"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option v-for="value in times[job.garbage.timeframe]" :key="'svcstuff' + value" :value="value">
                            {{ value }}</option>
                    </select>
                </div>
                <div>
                    <label for="svcd" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Amount of
                        Time</label>
                    <select id="svcd" v-model="job.garbage.timeframe"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="svcf" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Backup
                        Frequency</label>
                    <select v-model="job.schedule.frequency" id="svcf"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div>
                    <label for="svcs" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Backup
                        Time</label>
                    <TimePicker @change="handleChange"></TimePicker>
                </div>
            </div>
            <div class="relative mb-6" v-if="job.schedule.frequency !== 'days'">
                <label for="svcd" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Backup
                        Day</label>
                    <select v-model="job.schedule.day" id="svcd"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
...
export default {
    ...
    methods: {
        ...
        async save(e) {
            ...
            this.job.id = "id" + Math.random().toString(16).slice(2)

            this.job.createdat = new Date().toISOString()



            let res = await window.api.createJob(JSON.stringify(this.job))
