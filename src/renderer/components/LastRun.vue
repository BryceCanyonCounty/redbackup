<template>
    ...
    <div class="relative overflow-x-auto sm:rounded-lg">
        <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Recently Run
                </h5>
                ...
                <tr v-for="(rjob, index) in recentjobs" :key="rjob.id + index">
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        <div class="pl-3">
                            ...
                            <div class="font-small text-gray-500">{{ formatLastRun(rjob.lastrun) }}</div>
                        </div>
                    </th>
                    <td class="px-6 py-4">
    ...
    computed: {
        recentjobs() {
            return _.filter(this.jobs, (job) => {
                const date1 = dayjs()
                return date1.diff(job.lastrun, 'hour', true) < 4
            })
        }
    },
    methods: {
